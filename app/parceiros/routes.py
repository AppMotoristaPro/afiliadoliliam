...
from ..models import ParceiroConfig, Venda, LinkGerado
...
def dashboard():
    ...
    # Soma total para os novos cards de visão estratégica do afiliado
    ...
    return render_template('parceiros/dashboard.html', 
                           config=config, 
                           vol_vendas=vol_vendas, 
                           vol_comissoes=vol_comissoes)
...
def links():
    ...
    if request.method == 'POST':
        ...
        if original:
            ...
            # Correção aplicada: endswith minúsculo exigido pelo Python
            if base.endswith('/'): 
                base = base[:-1]
                
            rastreada = f"{base}/?utm_campaign={config.codigo_utm}"
            
            novo_link = LinkGerado(
                parceiro_id=config.id, 
                url_original=original, 
                url_rastreada=rastreada
            )
